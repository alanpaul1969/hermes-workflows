"""
Workflow Engine — YAML parser + prompt builder for Hermes workflow execution.

Translates declarative workflow YAML into Hermes agent prompts that use
delegate_task + skills to execute multi-agent orchestrated pipelines.
"""
...
def validate_workflow(data: dict) -> List[str]:
    ...
    for i, step in enumerate(data["steps"]):
        ...
        if stype not in STEP_TYPES:
            errors.append(f"{prefix}: unknown type '{stype}' (must be one of: {', '.join(STEP_TYPES)})")
...
def resolve_variables(template: str, variables: dict, step_outputs: dict = None) -> str:
...
def execute_subworkflow_step(step: dict, session: WorkflowSession) -> str:
    """Load and execute a referenced workflow YAML."""
    import json as _json
    ref_name = step["workflow"]
    wdir = Path(TEMPLATES_DIR).parent  # ~/.hermes/workflows/
...
def execute_gbrain_step(step: dict, session: WorkflowSession) -> str:
    """Run a gbrain step locally (query/search/put/get)."""
    import subprocess as _sp
    import json as _json
    ...
    if action == "query":
        ...
        r = _sp.run(["gbrain", "query", query], capture_output=True,
                     text=True, timeout=120, cwd=workdir)
    elif action == "search":
        query = session.resolve(step["query"])
        r = _sp.run(["gbrain", "search", query], capture_output=True,
                     text=True, timeout=120, cwd=workdir)
    elif action == "put":
        slug = session.resolve(step["slug"])
        content = session.resolve(step["content"])
        r = _sp.run(["gbrain", "put", slug, content], capture_output=True,
                     text=True, timeout=120, cwd=workdir)
    elif action == "get":
        slug = session.resolve(step["slug"])
        r = _sp.run(["gbrain", "get", slug], capture_output=True,
                     text=True, timeout=120, cwd=workdir)
    else:
        raise ValueError(f"Unknown gbrain action: {action}")
...
def resolve_execution_order(steps: List[dict]) -> List[List[str]]:
    ...
    while remaining:
        ...
        for sid in sorted(remaining):
            ...
            # Check parallel_group constraint
            pg = step.get("parallel_group")
            
            if all(d in completed for d in deps):
                # Check if any step in the same parallel_group is already in a
                # different wave (shouldn't happen if deps are correct)
                wave.append(sid)
...
def build_workflow_prompt(workflow: dict) -> str:
    ...
    opencode/workflow steps; defers subagent/skill steps to Hermes.

    Returns a prompt string for Hermes to execute remaining steps, or empty
    string if all steps were executed natively.
    """
    try:
    ...
    # Fallback: pure prompt-based execution (original behavior)
    ...
    for wave_idx, wave in enumerate(waves):
        label = f"Wave {wave_idx + 1} (parallel — {len(wave)} steps)" if len(wave) > 1 else f"Step {wave_idx + 1}"
        steps_text.append(f"\n### {label}")
        ...
        for sid in wave:
            ...
            if stype in ("subagent", "reasonix", "opencode"):
                ...
                agent_type = stype
                steps_text.append(f"  Use {agent_type} with toolsets={toolsets}:")
    ...
    return f"""Execute the following workflow using delegate_task for each subagent step.
...
2. For subagent steps, use delegate_task with exact context provided
3. Steps in same wave can run in parallel via delegate_task batch mode
...
TEMPLATES_DIR = os.path.expanduser("~/.hermes/workflows/_templates")


def load_template(name: str) -> dict:
    """Load a workflow template by name. Returns dict with 'content' (raw text) and 'data' (parsed YAML)."""
    ...
    if not os.path.exists(path):
        ...
        if os.path.isdir(tdir):
            available = sorted(f.replace(".yaml", "") for f in os.listdir(tdir) if f.endswith(".yaml"))
        
        msg = f"Template '{name}' not found in {TEMPLATES_DIR}"
        if available:
...
def get_available_templates() -> list[str]:
    """Return list of available template names from _templates/ directory."""
    if not os.path.isdir(TEMPLATES_DIR):
        return []
    
    return sorted(f.replace(".yaml", "") for f in os.listdir(TEMPLATES_DIR) if f.endswith(".yaml"))


def instantiate_template(template_data: dict, variables: dict) -> str:
    """Replace all {KEY} placeholders in the template content with values from variables dict."""
...
def classify_task(description: str) -> dict:
    ...
    if all(kw in desc_lower for kw in ("flutter", "bug")) or \
       (desc_lower.count("flutter") > 0 and any(kw in desc_lower for kw in ("bug", "fix", "crash"))):
        return {"template": "flutter-bug-fix", "variables": {"TASK_DESCRIPTION": description}}
    
    if desc_lower.count("flutter") > 0 and any(kw in desc_lower for kw in ("feature", "new", "add")):
        return {"template": "flutter-feature", "variables": {"TASK_DESCRIPTION": description}}
    ...
    if has_backend and has_bug:
        return {"template": "backend-bug-fix", "variables": {"TASK_DESCRIPTION": description}}
    
    if has_backend and has_feature:
        return {"template": "backend-feature", "variables": {"TASK_DESCRIPTION": description}}
...
def execute_workflow(workflow: dict) -> dict:
    ...
    ``provider: hermes`` to defer them for external execution.
    Skill steps are always deferred.

    Returns:
        {"local_outputs": {step_id: output}, "deferred_steps": [...], "prompt": str}
    """
    steps = workflow.get("steps", [])
    variables = workflow.get("variables", {})
    ...
    for wave in waves:
        for sid in wave:
            ...
            if stype == "command":
                ...
                execute_subworkflow_step(step, session)
            elif stype == "gbrain":
...
def build_deferred_prompt(workflow: dict, deferred_steps: list, session: WorkflowSession) -> str:
    """Build a Hermes prompt for deferred subagent/skill steps with outputs injected."""
    name = workflow.get("name", "Unnamed Workflow")
    description = workflow.get("description", "")
    ...
    for i, step in enumerate(deferred_steps):
        ...
        if stype == "subagent":
            ...
            parts.append(f"Use delegate_task with toolsets={toolsets}:")
            parts.append(f"```\n{ctx}\n```")
        elif stype == "skill":
            skill_name = step.get("skill", "")
            parts.append(f"Load skill: `{skill_name}` and execute with input: {step.get('input', {})}")

        parts.append("")

    parts.append("Execute these steps in order using delegate_task.")
    return "\n".join(parts)
...
def list_workflows(workflows_dir: str = "~/.hermes/workflows") -> List[dict]:
